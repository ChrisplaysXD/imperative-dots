...
Item {
    ...
    Process {
        id: themeReader
        // Kalo ada music_colors.json (dari album art) kita pake, kalo ga ada fallback ke qs_colors.json bawaan wallpaper
        command: ["bash", "-c", "cat /tmp/eww_covers/music_colors.json 2>/dev/null || cat $HOME/.config/hypr/scripts/quickshell/qs_colors.json 2>/dev/null"]
    ...
    Timer {
        interval: 800 // Lebih cepat sedikit biar responsif pas ganti lagu
        running: true
        repeat: true
